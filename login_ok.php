...
if( $userID && $userPassword != "") // 공백 체크
{
  ...
  include('DB/Connection.php'); //DB 연결 소스코드 불러오기
  ...
  $res_pw = mysqli_fetch_array(mysqli_query($db_con, $DB_PW)); // DB에서 입력한 아이디값에 해당하는 암호화된 패스워드 가져오기
  $res = mysqli_fetch_array(mysqli_query($db_con, $DB_ID) ); // 입력값에 해당하는 아이디에 대한 컬럼값을 배열로 가져오기
